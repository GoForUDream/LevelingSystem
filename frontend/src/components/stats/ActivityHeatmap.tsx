...
export default function ActivityHeatmap({ data }: ActivityHeatmapProps) {
  // Group data by week for grid layout
  ...
  return (
    <div className="led-border p-4 border border-green-400/30 bg-linear-to-b from-sl-gray-light/30 to-sl-gray/20" style={{ '--led-color': '#4ADE80' } as React.CSSProperties}>
          ...
          <div className="flex items-center pl-6 gap-0.5 mb-1">
            {monthLabels.map((m, i) => (
              <div
                ...
                style={{
                  marginLeft: i === 0 ? `${m.weekIndex * 12}px` : `${(m.weekIndex - (monthLabels[i - 1]?.weekIndex ?? 0) - 1) * 12}px`,
                }}
              >
                {m.label}
              </div>
                  ...
                  <Tooltip key={`${weekIndex}-${dayIndex}`}>
                    ...
                    {day.date && (
                      ...
                      >
                        ...
                        <p className="text-[10px] text-sl-silver-muted">
                          {formatDate(day.date)}
                        </p>
                      </TooltipContent>
                    )}
...
function getIntensityColor(intensity: number): string {
  ...
  return colors[intensity as keyof typeof colors] || 'transparent';
}
